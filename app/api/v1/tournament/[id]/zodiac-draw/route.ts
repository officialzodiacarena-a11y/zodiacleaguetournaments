//app/api/v1/tournaments/[id]/zodiac-draw/route.ts
...
import {
  ZODIAC_SIGNS,
  ZodiacDrawRequestSchema,
  type ZodiacDrawEntry,
  type ZodiacDrawResult,
} from '@/types/finals';
...
export async function GET(
  ...
  try {
    ...
    const { data: rawTournament, error: tourError } = await supabase
    ...
    const tournament = rawTournament as unknown as TournamentQueryResult;
    const formatConfig = ((tournament.format_config ?? {}) as Record<string, unknown>);
    const draw = (formatConfig.zodiac_draw as ZodiacDrawResult) ?? null;
...
export async function POST(
  ...
  try {
    ...
    const tournament = rawTournament as unknown as TournamentQueryResult;
    const existingConfig = ((tournament.format_config ?? {}) as Record<string, unknown>);
    ...
    if (qualifiedTeams.length !== 12) {
      return NextResponse.json(
        {
          error: {
            code: 'INSUFFICIENT_QUALIFIED_TEAMS',
            message: `ต้องมีทีมผ่านสิทธิ์ finals_seed ครบ 12 ทีมก่อน (ปัจจุบัน ${qualifiedTeams.length} ทีม) — รัน /admin/finals/circuit-lock ก่อน`,
          },
        },
        { status: 422 }
      );
    ...
    await adminSupabase.from('audit_logs' as never).insert({
      ...
      after_data: drawResult as unknown as Json,
    } as never);
