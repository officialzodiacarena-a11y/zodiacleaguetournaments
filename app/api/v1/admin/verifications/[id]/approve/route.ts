...
export async function PATCH(
  ...
  try {
    ...
    if (roleError || !userRole || !allowedRoles.includes(userRole.role)) {
      return NextResponse.json(
        { error: { code: 'FORBIDDEN_ROLE', message: 'บัญชีนี้ไม่มีสิทธิ์อนุมัติการยืนยันตัวตน' } },
        { status: 403 }
      );
    }
    ...
    if (gameAccount.verification_status !== 'PENDING' && gameAccount.verification_status !== 'MANUAL_REVIEW') {
      return NextResponse.json(
        { error: { code: 'INVALID_STATE_TRANSITION', message: `ไม่สามารถอนุมัติจากสถานะ ${gameAccount.verification_status} ได้ ต้องเป็น PENDING เท่านั้น` } },
        { status: 409 }
      );
    }
    ...
    const { data: updated, error: updateError } = await adminSupabase
      ...
      .eq('id', gameAccountId)
      .select('id, player_id, verification_status, verified_at')
      .single();
    ...
    // อนุมัติแล้ว = ข้อมูลเชื่อถือได้อีกครั้ง เคลียร์ธง unverified_data ที่อาจค้างจาก revoke ครั้งก่อน
    await adminSupabase
      .from('players')
    ...
    await adminSupabase.from('audit_logs').insert({
      ...
      after_data: { verification_status: 'VERIFIED', player_id: gameAccount.player_id },
    });

    await adminSupabase.channel(`player-presence-${gameAccount.player_id}`).send({
      type: 'broadcast',
      event: 'verification_updated',
      payload: { player_id: gameAccount.player_id, verification_status: 'VERIFIED' },
    });

    return NextResponse.json({
      success: true,
      message: 'อนุมัติการเชื่อมโยงบัญชีและปรับสถานะผู้เล่นเป็น VERIFIED เรียบร้อย',
      data: { ...updated, reviewed_by: admin.id },
    });
  } catch (error: unknown) {
